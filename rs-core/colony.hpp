...
// [RS] None of the compilers I care about have any problem with any of these,
// and the original tests were out of date anyway.
...
public:
    ...
    void change_group_sizes(const skipfield_type min_allocation_amount, const skipfield_type max_allocation_amount)
    {
        ...
        // [RS] Workaround for MSVC 2017 parser bug
        // if (first_group != NULL && (first_group->size < min_allocation_amount || end_iterator.group_pointer->size > max_allocation_amount))
        bool first_lt = first_group->size < min_allocation_amount;
        bool last_gt = end_iterator.group_pointer->size > max_allocation_amount;
        if (first_group != NULL && (first_lt || last_gt))
        {
            consolidate();
        }
    }
