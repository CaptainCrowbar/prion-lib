...
namespace RS {
    ...
    public:
        ...
        void encode(std::string_view src, Ustring& dst, size_t width = npos) const { encode_bytes(src.data(), src.size(), dst, width); }
        Ustring encode(std::string_view src, size_t width = npos) const { Ustring dst; encode_bytes(src.data(), src.size(), dst, width); return dst; }
    ...
    public:
        ...
        virtual size_t decode_bytes(const char* src, size_t len, std::string& dst) const {
    ...
    public:
        virtual void encode_bytes(const void* src, size_t len, Ustring& dst, size_t width = npos) const {
            ...
            for (;;) {
                if (i == len || j == 5) {
                    size_t n_out = (8 * j + 4) / 5;
                    for (size_t k = 0; k < n_out; ++k) {
                        if (width > 0 && column++ == width) {
                            dst += '\n';
                            column = 1;
                        }
                        size_t digit = group & 0x1f;
                        if (digit < 10)
                            dst += char('0' + digit);
                        else
                            dst += char('a' + (digit - 10));
                ...
                group |= uint64_t(bytes[i++]) << (8 * j++);
            }
        }
        virtual size_t decode_bytes(const char* src, size_t len, std::string& dst) const {
            size_t i = 0, j = 0;
            uint64_t digit = 0, group = 0;
            for (;;) {
                if (i == len || j == 8) {
                    size_t n_out = 5 * j / 8;
                    for (size_t k = 0; k < n_out; ++k) {
