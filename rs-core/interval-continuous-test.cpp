...
void test_core_interval_continuous_arithmetic() {
    ...
    static const std::vector<test_info> tests = {
        ...
        { __LINE__,  "(10,20]",  "[-86,99]",  "(-76,119]",  "(-89,106]",  "[-106,89)",  "{}",  "{}",  "{}",  },
        { __LINE__,  "(10,20)",  "[-86,99]",  "(-76,119)",  "(-89,106)",  "(-106,89)",  "{}",  "{}",  "{}",  },
        { __LINE__,  "<10",      "[-86,99]",  "<109",       "<96",        ">-96",       "{}",  "{}",  "{}",  },
        { __LINE__,  "<=10",     "[-86,99]",  "<=109",      "<=96",       ">=-96",      "{}",  "{}",  "{}",  },
        { __LINE__,  ">10",      "[-86,99]",  ">-76",       ">-89",       "<89",        "{}",  "{}",  "{}",  },
        { __LINE__,  ">=10",     "[-86,99]",  ">=-76",      ">=-89",      "<=89",       "{}",  "{}",  "{}",  },
        { __LINE__,  "[10,20)",  "[-86,99)",  "[-76,119)",  "(-89,106)",  "(-106,89)",  "{}",  "{}",  "{}",  },
        { __LINE__,  "(10,20]",  "[-86,99)",  "(-76,119)",  "(-89,106]",  "[-106,89)",  "{}",  "{}",  "{}",  },
        { __LINE__,  "(10,20)",  "[-86,99)",  "(-76,119)",  "(-89,106)",  "(-106,89)",  "{}",  "{}",  "{}",  },
    ...
    for (auto& t: tests) {
        ...
        TRY(c = a - b);           TRY(str = to_str(c));  TEST_EQUAL(str, t.sub1);  errors += int(str != t.sub1);
        TRY(c = b - a);           TRY(str = to_str(c));  TEST_EQUAL(str, t.sub2);  errors += int(str != t.sub2);
        TRY(c = a * b);           TRY(str = to_str(c));  TEST_EQUAL(str, t.mul);   errors += int(str != t.mul);
        TRY(c = a / b);           TRY(str = to_str(c));  TEST_EQUAL(str, t.div1);  errors += int(str != t.div1);
        TRY(c = b / a);           TRY(str = to_str(c));  TEST_EQUAL(str, t.div2);  errors += int(str != t.div2);
        TRY(c = a); TRY(c += b);  TRY(str = to_str(c));  TEST_EQUAL(str, t.add);   errors += int(str != t.add);
        TRY(c = a); TRY(c -= b);  TRY(str = to_str(c));  TEST_EQUAL(str, t.sub1);  errors += int(str != t.sub1);
        TRY(c = b); TRY(c -= a);  TRY(str = to_str(c));  TEST_EQUAL(str, t.sub2);  errors += int(str != t.sub2);
        TRY(c = a); TRY(c *= b);  TRY(str = to_str(c));  TEST_EQUAL(str, t.mul);   errors += int(str != t.mul);
        TRY(c = a); TRY(c /= b);  TRY(str = to_str(c));  TEST_EQUAL(str, t.div1);  errors += int(str != t.div1);
        TRY(c = b); TRY(c /= a);  TRY(str = to_str(c));  TEST_EQUAL(str, t.div2);  errors += int(str != t.div2);
